...
- **渲染体验**：LaTeX 公式（KaTeX）、`thinking` 代码块折叠为思考过程、`mermaid` 代码块渲染流程图（跟随深浅主题，切换主题自动重渲染）、GFM 任务列表 checkbox、消息时间显示、流式打字机光标、消息平滑淡入
- **文件引用卡片**：消息中的 `wb://路径` 独立行渲染为文件引用卡片，点击直接在侧栏预览工作区文件
